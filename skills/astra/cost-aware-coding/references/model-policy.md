...
judge. The point of Luna here is to retain strong bounded-task intelligence at very
low marginal cost, not to minimize Luna reasoning effort.

Cheap inference is not a cheap workflow when Astra must provide a large brief,
reconstruct missing context, or deeply verify the output. Do not split one coherent
Sol assignment into many Luna tasks merely to reduce per-token price.

## Effort escalation

Start with Astra Medium and Sol Medium. Raise effort only when evidence suggests
that more reasoning can change the result:
...
- Keep bounded Luna work on **Luna Max**. If the task outgrows a compact bounded
  contract, move it to Sol rather than treating Luna effort as a tuning ladder.

Prefer the cheapest route that reliably meets acceptance. Repeated failure at a
lower-capability route is not useful savings when it creates expensive briefing,
