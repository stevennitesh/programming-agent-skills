...
**Integrate.** Incorporate the answer, preserve material alternatives and deferrals, reopen invalidated decisions, and classify new branches as prerequisites, deferrals, or Bound-authority scope changes. Continue `Find -> Grill -> Integrate` until every in-bound material branch is resolved or validly deferred. Permit one focused clarification; return an authority gap instead of rephrasing indefinitely.
...
**Return.** Return `Confirmed` or `Evidence gap`, the bound, confirmed decisions, populated supporting details, return owner, and `Downstream execution: none`. Return to the caller or user and stop.
