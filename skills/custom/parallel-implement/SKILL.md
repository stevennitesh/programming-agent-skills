...
Apply the **setup gate**: read `docs/agents/engineering-contract.md`; when tracker work is in scope, read `docs/agents/issue-tracker.md` and what it points to. If a required setup surface is missing, stop and recommend `$setup-matt-pocock-skills`. Read `docs/agents/domain.md` only when domain semantics affect the run.
