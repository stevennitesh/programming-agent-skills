...
2. **Manual Git:** select an explicit configured root, then `<repo-parent>/worktrees/parallel-implement`, then an environment-declared auxiliary writable root.
...
Omit `--root` for the short repo-parent default. Pass `--branch <name>` only for a routed integrator. The helper uses a bounded hashed lane name, verifies containment, budgets generated and tracked path space, creates detached workers by default, verifies Git registration and base, and reports command-scoped Git trust.
...
Proof startup is required. When no repository startup proof exists, use `--skip-proof --reason <reason>` and carry that skip as residual risk.
