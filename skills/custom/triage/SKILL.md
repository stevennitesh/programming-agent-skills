...
3. **Classify.** Keep exactly one mapped category role and one mapped state role; surface missing or conflicting roles before mutation. Normal intake moves from unclassified to `needs-triage`, then to `needs-info`, `ready-for-agent`, `ready-for-human`, or `wontfix`. Reporter activity returns `needs-info` to `needs-triage`. Treat valid `$to-tickets` output as ready; re-enter triage only for an explicit state or brief correction.

4. **Approve.** Mutate only the maintainer-approved packet. A `ready-for-agent` outcome satisfies the tracker's Ready-for-agent contract and adds verification evidence; record any explicit label-only override in the triage packet.

5. **Apply.** Follow the chosen branch. Prefix every posted issue or comment with:

   ```markdown
   > *This was generated by AI during triage.*
   ```

6. **Prove.** A scan ends at its read-only criterion. A mutation ends only after the approved packet is applied, read back, and returned as a triage packet confirming role invariants, disclaimer, required brief or rejected-enhancement record, shaping domain deltas, partial failures, skipped checks, and blockers.
