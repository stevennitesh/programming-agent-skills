...
1. **Admit.** Accept only a direct-user request when the decision needs both
   components. Align their bounded subject and Source Trace. Default the context
   action to `render only` unless the user separately authorizes persistence;
...
2. **Compose.** Run one `$grilling` session with `$domain-modeling` active.
   Relay every settled material answer and returned collision before dependent
   progress, carrying Domain Modeling's current cumulative Domain Delta
   opaquely. Grilling owns the interview and materiality; Domain Modeling owns
   domain consequences, mutation, ADR handling, and delta accumulation. A
...
3. **Return.** When Grilling reaches a terminal result, derive one status
   ...
   uninvoked evidence owner. `Blocked` explains re-entry without performing
   recovery.

## Completion

Complete only when authority was disclosed before questioning, every settled
material answer and returned collision traversed Relay before dependent
progress, the component payloads are current and intact, the Return has exactly
one allowed status, every blocker has an exact re-entry condition, and
