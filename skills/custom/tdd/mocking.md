...
Keep owned in-process modules behind the tested interface real. Put a test
double only at a real boundary adapter.
...
double and the real or local implementation; otherwise record the unverified
fidelity risk.

Reconsider the seam when fidelity is unclear or double setup overwhelms the
