...
| `parallel-implement` | `WORKER-BRIEF.md`: implementation, correction, and Repair Return contract; `AGENT-LANES.md`: subagent dispatch and checkout isolation; `assets/luna_max.toml`: canonical named-agent template; `lane_worktree.py`: isolated worktree lifecycle; `run_ledger.py` and `RUN-LEDGER.md`: campaign events, validation, generated ledger, and closeout plan |
