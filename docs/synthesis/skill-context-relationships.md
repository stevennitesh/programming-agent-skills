...
| `to-tickets` | Recommend and stop | `$codebase-design` | Before Slice, repository grounding exposes one bounded consequential implementation-architecture choice within the settled source. Return the question, relevant evidence, and re-entry condition with graph drafting and tracker mutation unstarted. |
...
| `codebase-design` | One bounded module or interface architecture decision using deep-module, caller-first, data-shape, ownership, seam, migration, and proof judgment | `audit-codebase`, `to-tickets` unresolved-architecture branch, direct architecture/design work |
