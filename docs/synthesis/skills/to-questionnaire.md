...
| Current user | Exclude as current-user-owned and name `$grilling` for a direct conversation-only mismatch or `$grill-with-docs` when repo-backed durable domain capture must remain active; delegated runs return the classification to the caller |
...
| `I1,I2 / E1` | `Q1,Q2` | [Invocation And Routing](#invocation-and-routing) | Direct all-source, all-user, or no-path mismatches recommend Research, the correctly admitted Grilling entry, or Repo Bootstrap and stop; delegated mismatch returns classification only | A repo-backed domain decision recommends conversation-only Grilling, a conversation-only decision starts the composer, a delegated run recommends or invokes another skill, or the relationship map omits an executable edge | Direct/delegated paired cases and relationship-map test |
