...
| `$to-questionnaire` | One identifiable external stakeholder holds one knowledge gap and needs one recipient-ready questionnaire for asynchronous completion or a later meeting, with one downstream decision or prerequisite and needed-back ledger | Public sources can answer it, the current user owns the answer, there are several stakeholder groups or downstream needs, the request is to conduct the interview, or sending/answer collection is the requested work |
...
- A Grilling Evidence gap inside Grill With Docs returns through the composer with the current Domain Delta; it does not re-enter Grill With Docs or ask Skill Router to confirm the named owner.
...
| Runtime callers | Wayfinder and Domain Modeling syntheses select future complete-residual callers; Prototype explicitly returns every non-admission to its caller or the user and is not a residual-Router caller | Audit and promote each selected caller edge atomically; do not claim generic caller adoption |
...
| `I2-I3 / E1,E3` | `R4,R5` | [Relationship Contract](#relationship-contract) | Known handoffs stay local; Wayfinder and standalone Domain Modeling invoke Router only with their proved complete terminal residuals, while Prototype always returns locally | Callers send owned or unfinished edges through Router, Prototype delegates a non-admission, or any caller duplicates the complete route catalog | Exhaustive active-surface relationship audit |
