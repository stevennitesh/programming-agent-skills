# Frozen Wayfinder State-Machine Proposal

Historical design evidence only. This file is not runtime, relationship,
compatibility, extraction, or promotion authority. See
[wayfinder.md](wayfinder.md) for the current synthesis and
`skills/custom/wayfinder/` for runtime behavior.

Status: exhaustive design reference, not an executable contract.

Snapshot note: present-tense paths and promotion claims below describe this
frozen proposal. The experimental package was later retired, and the lean
current runtime independently adopted Terminate.
...
not assessed in this pass. The retired candidate and its `OPERATIONS.md`
extraction remain under `skills/.archive/wayfinder/` as history only.
