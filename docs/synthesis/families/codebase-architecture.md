...
- after explicit selection, conditionally research, prototype, or design; when the selected candidate needs both a user-owned decision and durable domain capture, recommend explicit `$grill-with-docs` and stop, then resume that candidate in a later invocation with the direct-user result
