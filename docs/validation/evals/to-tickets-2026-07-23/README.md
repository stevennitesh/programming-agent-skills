...
Deploy Prompt 4 resumed with evaluator material removed from worker packets.
The clean continuation is frozen under [`isolation-v2/`](isolation-v2/).
Its terminal decision is `accepted`; all three H1 units were rejected, and V1
is exact M0 package
...
- `fixtures/runtime.json` fixes callable model, reasoning, host, isolation,
  sample, tool, authority, and capture policy.
- `fixtures/tracker-contract.md`, `fixtures/m0.json`, and `fixtures/h1.json`
  freeze the complete worker context and evidence for every registered case.
