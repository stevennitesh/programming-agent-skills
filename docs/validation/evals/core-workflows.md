...
**Required:** the baseline request routes to explicit `$audit-codebase` and completes one of three user objectives: Map, Audit, or Analyze. Map observes current repository provenance, maps every system and subsystem, assigns every shared unit one audit-owning subsystem plus named consumers, atomically publishes the sole durable linked HTML atlas under `.scratch/audit-codebase/`, and stops for user selection. It emits report version 4 with static subsystem containers, sibling narrative, finding, candidate-index and candidate-card regions, and insertion anchors. A supplied report passes the contained-path, version, repository/run, unique-selection, collision, and immediate representation gates; an invalid or structurally unsupported explicit selection is never replaced with Map or analyzed first. Audit refreshes one selected subsystem's current Source Trace, always loads Reliability, Quality, and Defect owners, records all six mandatory lens dispositions, loads detailed Domain, Design, Simplification, Coding Practice, Performance, and declared owners when implicated, verifies every obtainable in-scope branch, and distinguishes defects, opportunities, retained complexity, and gaps. Its narrative renders machine-readable retained-complexity, gap, and opportunity collections with unique IDs and subsystem ownership. Analyze loads the candidate core plus implicated owners, rereads current source before confirming, changing, disproving, or blocking the candidate, treats a ready unsent questionnaire as an intermediate artifact rather than evidence, and folds `$codebase-design` Direct Design into a settled design or mixed candidate. Decision, returned-evidence, and successor routing load only when their branch applies. Audit validates one versioned publication manifest through `reaudit-subsystem --validate-only`, then publishes the unchanged digest-locked bundle once; Analyze uses generic `validate`, then `update` for existing sibling regions. The same `reaudit-subsystem` engine refreshes narrative, upserts findings, and updates or inserts candidates. The helper may canonicalize an explicit evidence path list but never discovers audit scope. It derives candidate and finding summaries, and errors distinguish stage, mutation start, and whether the report changed. A publication failure preserves the report, triggers no retry, hand edit, or alternate mechanism, and returns completed source analysis immediately with `Publication result: failed`. Neither objective ranks subsystems or starts downstream work. The ordinary PR routes to `$change-review`, the supported high-risk PR to `$high-assurance-review`, the bounded behavior-preserving edit to `$simplify-code`, and the uncertain symptom to `$diagnosing-bugs`.
