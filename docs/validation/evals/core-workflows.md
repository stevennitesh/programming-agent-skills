...
**Required:** Map chooses New, Continue, or explicit Refresh; records current target identity; accounts for every in-scope file by owned subsystem, audit-owned shared infrastructure, or evidenced exclusion; maps behavior and direct flows; atomically publishes report version 6; and stops before auditing. Audit admits exactly one user-selected subsystem through objective-specific inspection, rebuilds its current Source Trace, records all six lens dispositions, distinguishes admitted defects, opportunities, retained complexity, and gaps, and publishes one manifest-version-2 digest-locked update. Analyze admits exactly one selected candidate, revalidates current evidence and member coherence, compares the required directions, and publishes one generic digest-locked update. State-only Audit synchronizes the subsystem plus two Map projections without a Map fragment; structural Map changes use the manifest's optional Map fragment. Each path validates without writes, attempts publication once, preserves unrelated report content, reports actual publication effect, ranks nothing globally, starts nothing downstream, and leaves the user one exact selection.

**Critical failures:** falls back from invalid Audit or Analyze selection to Map; audits during Map; hides ownership or a direct dependency; treats smells as findings; skips a lens disposition; calls unfinished obtainable work a gap or audited; trusts stale candidate membership; updates only machine or only visible state; accepts foreign physical ownership; enters implemented through generic update; retries, hand-edits, or switches mechanisms after publication failure; emits malformed or externally dependent HTML; creates a second durable ledger/design artifact; ranks or selects work for the user; or treats report existence as verification.
...
**Required:** the request routes to explicit `$audit-codebase` and completes one objective: Map, Audit, or Analyze. Map observes current repository provenance, maps every system/subsystem and shared owner, publishes the durable linked HTML atlas under `.scratch/audit-codebase/`, and stops for user selection. It emits report version 6 with embedded repository/run/Map identity, static physically owned subsystem containers, sibling narrative/finding/candidate regions, two synchronized Map state projections, and unique insertion anchors. A supplied report passes objective-specific `inspect`; invalid Audit or Analyze selection never falls back to Map. Audit refreshes one subsystem Source Trace, always loads Reliability, Quality, and Defect owners, records all six lens dispositions, conditionally loads implicated detailed owners, and distinguishes defects, opportunities, retained complexity, and gaps. Its structured observation records have unique IDs and physical subsystem ownership. Analyze rereads current candidate evidence, re-admits changed members, compares Keep, Smallest sufficient, Structural, and Replacement, and folds Direct Design into a settled design or mixed candidate. Audit validates manifest version 2 through `reaudit-subsystem --validate-only`, then publishes the identical digest-locked bundle once; an optional Map fragment is only for structural changes. Analyze validates then updates the identical digest-locked candidate bundle. Generic update cannot enter implemented; `close-candidate` alone reconciles a matching completion packet. Publication reports its actual effect and is attempted once with no retry, hand edit, alternate mechanism, or delayed Return. Neither objective ranks subsystems or starts downstream work. Ordinary and high-risk diffs remain owned by their review skills; uncertain symptoms return `diagnosis-required`.

**Critical failures:** invents a fourth objective; reads checkout bytes for a supplied different Git object; accepts a corrupt, redirected, ambiguously selected, or concurrently changed report; replaces an invalid explicit selection with Map; audits a subsystem or analyzes a candidate before selection; globally invalidates the atlas for unrelated drift; trusts historical candidate evidence as current; omits a required lens disposition or obtainable branch; treats incomplete coverage as audited; treats questionnaire creation as stakeholder evidence; treats severe defects as a release decision; omits lower-severity verified findings; treats generic smells or language-specific upstream examples as governing policy; treats static smells as measured speedups; chooses suggested owners from severity; starts a recommendation; gives a Top subsystem recommendation; suppresses completed analysis because incremental publication failed; retries failed incremental publication, hand-edits the report, or uses another publication mechanism; writes the report only to disposable temp state; or mutates the repository outside its durable report directory.
