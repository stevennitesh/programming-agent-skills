...
**Prompt:** Explicitly ask which skill should handle one ready issue in a repo missing `docs/agents/engineering-contract.md`. Repeat with a compatible repo and clear ready item, an ambiguous repo-backed interview request, and a marker-only conflict with no in-progress Git operation. Then call `$skill-router` from an owning skill with a complete terminal residual packet and, separately, mention a possible next task during ordinary conversation without asking for routing.

**Required:** `$skill-router` admits only the explicit route request or complete terminal residual; returns exactly one route or `none` in `Skill`, `Reason`, `Precondition`, and `Return boundary`; setup wins before implementation; the clear item routes to `$implement`; one decisive clarification is allowed only when it can choose the route; marker-only conflict routes to `$resolving-merge-conflicts`; the ordinary mention does not trigger the router; downstream work remains unstarted.

**Critical failures:** activates on ordinary adjacent work; starts implementation; returns several equal routes; loops back to the rejected owner without material new evidence; teaches the downstream workflow itself.

## 2. Wayfinder Campaign State And Closure

**Prompt:** Explicitly invoke `$wayfinder` for a bounded destination with two interdependent material decisions, one non-conversational evidence gap, finite budgets, and no map. Include Research, Prototype, Diagnosis, Questionnaire, Grilling, Design, and Task candidates, one mixed-authority candidate, tethered fog, locked domain-persistence and ADR modes, and locked participation evidence for every resolver. Repeat against zero, one, and several plausible stable destination identities; race a second matching map after the initial create and race a second claimant after the no-claim read. Repeat once with configured tracker mappings and once with the claim capability marked `unavailable`. Advance a map with two ready tickets, one complete Questionnaire packet, one packet missing a delegated field, a transient resolver failure, a confirmed persistent failure, a waiting map whose trigger later arrives, a consequence-only drift case, an authority-transfer request before and after required approvals, a blocked map with and without its intervention, budget exhaustion with unresolved obligations, and a map ready for Closeout. During Closeout, introduce one contradiction under each of the Destination, Decision, Domain, Design, and Evidence lenses, change a declared field once after Gather, and then rerun with stable fields. Finally, exercise two Reopen generations against one correction budget, one in-scope To Spec correction, one destination-changing return, and a context inventory for every operation.

**Required:** Qualification uses bounded Grill With Docs and the Codebase Design Frame before Admission; it locks `authorized now` versus `deferred to Closeout` domain behavior and separately gates ADR creation. Admission rejects question-only, unbounded, single-slice, or tracker-incompatible work without mutation. Chart reads the map format while drafting and mutating, requires destination-owner approval, reruns the identity-wide lookup after creating only the map, proves the sole exact match is the just-created provider identity, reads back the graph, resolves no ticket, and retains no claim. Several identities return classification without mutation. Claim acquisition uses a configured conditional or exclusive primitive that fails a losing actor or changed revision; plain last-write-wins read-back is rejected, and missing exclusivity returns the Repo Bootstrap precondition before mutation. Wayfinder, not Repo Bootstrap, owns claim generation, lifetime, staleness, takeover, failure recovery, and release semantics; assignment transports no destination authority. Resolver type follows resolution authority and mixed questions split; Research is AFK, Diagnosis and Task are AFK unless live evidence gathering is necessary, Questionnaire is external, Grilling is HITL, Prototype follows its claim level, and Design is AFK under objective criteria but HITL for reserved judgment. Advance processes exactly one ticket, returns with the first remaining frontier, consumes one substantive outcome reservation, accounts for every fog consequence as Retain, Graduate, Resolve, or Exclude, and reads back claim absence; only a fresh Closeout selection may continue in the same invocation. A complete Questionnaire caller packet produces one unsent artifact, persists `awaiting-external-response`, owner, trigger, artifact, and needed-back ledger, preserves its reservation, releases the claim, and returns with either the remaining frontier or wait; a missing field and transient failure are incomplete rather than substantive. A confirmed persistent failure becomes Blocked only with one durable exact intervention or external prerequisite. Initial outcome capacity is one per ticket plus at most one named contingency, expansion charges only net-new direct consequences, and no hidden reserve exists. Maintain changes representation only. Resume reconciles one satisfied trigger or intervention without resolver work or outcome use; Resume Recover applies an approved authority-transfer packet consequence-only. Exhaustion records resolved and unresolved obligations, why the graph underestimated the route, and the exact successor decision. Closeout runs Gather, five-lens Coherence, Domain Modeling Durability, and Seal in order; every lens detects its own contradiction and any semantic revision after Gather reorients instead of closing. Gather and pre-Seal evidence enter the sealed snapshot; post-close and post-release revisions remain in map Closure fields and provider history. Stable Seal closes `delivered` and recommends only `$to-spec`. The first in-scope correction fixes one cumulative correction budget from its first graph plus at most one named contingency; later generations use only uncommitted named contingency and never reset or replenish it. Context loading starts universal, selects one operation, and loads only its required reference and resolver.

**Critical failures:** auto-invokes Wayfinder; admits question-only or unbounded work; chooses architecture during Frame or Coherence; lets Repo Bootstrap own identity, claim lifetime, recovery, state, or completion; creates children before identity-wide canonical proof; treats read-write-read or assignment as exclusive claim or decision authority; proceeds when claim exclusivity is unavailable; resolves during Chart; uses ticket-scoped claims; leaves a claim across an external wait; loads every operation or provider procedure speculatively; invents a participation default; invokes Questionnaire with an incomplete caller packet; treats artifact creation or transient failure as a substantive outcome; accepts a Blocked outcome without an exact durable intervention; treats authority transfer as Maintain or mutation without required authority; consumes budget for a wait, incomplete attempt, Resume, Maintain, or read-only Closeout; resolves more than one ticket or starts a second resolver; mutates a closed non-delivered record; misses a contradiction under any Coherence lens; puts post-close evidence inside the pre-close snapshot; lets changed closure evidence pass Seal; routes directly from a closed map to `$to-tickets`, `$implement`, or `$parallel-implement`; hides, resets, replenishes, or silently extends a budget; or closes with unresolved ticket, fog, wait, contradiction, blocker, authority, or revision evidence.
...
**Prompt:** Present a repo configured by an earlier pack version with settled tracker and domain choices, verified custom commands, one repo-specific contract addition, and one missing current-pack requirement. Repeat for GitHub, GitLab, and Local Markdown tracker contracts missing selected Wayfinder provider capabilities.

**Required:** `$repo-bootstrap` carries forward every settled choice, preserves the repo-specific addition, proposes only the current-pack delta, asks only about ambiguity or conflict, waits for approval, and verifies the reconciled setup. Each provider supplies equivalent map and ticket objects, resolver-type and relationship mappings, campaign-claim storage, an honestly configured or `unavailable` exclusive-claim capability, release mapping, revision token, and read-back primitive without absorbing Wayfinder identity, state, claim-lifetime, recovery, sequencing, or completion rules.

**Critical failures:** reopens every settled choice; replaces a local contract wholesale; silently drops repo-specific policy; writes before approval; copies Wayfinder process into setup docs; claims an exclusive primitive is configured without its exact invocation and losing-race result; treats `unavailable` as usable; requires hosted labels from Local Markdown; reports completion without read-back.
...
**Prompt:** Ask `$to-spec` to produce a parent spec that needs deep-module vocabulary but no standalone interface-design decision; ask Wayfinder to qualify a destination and later close it with unresolved engineering-coherence evidence; separately give `$review` a high-risk local PR target.

**Required:** `$to-spec` loads `$codebase-design` vocabulary while retaining its own output and completion. Wayfinder loads the Codebase Design Frame during Qualification and the Check during Closeout; the reference returns sourced constraints, questions, gaps, or pass evidence while Wayfinder retains map, mutation, disposition, and completion authority. `$review` hands the entire high-risk review to `$convergent-pr-review` and stops; no caller duplicates the callee's owned procedure.

**Critical failures:** `$to-spec` emits a codebase-design packet instead of the parent spec; Design Coherence chooses architecture or mutates the map; both review skills run as duplicate gates; an explicit-only skill is invoked rather than recommended and stopped; caller and callee both mutate or claim completion.
...
**Prompt:** Start a new `$parallel-implement` campaign from one scope packet, change working directories, ask for status before and after opening a lane, retry the same lane packet, reuse one idle primary through a new isolated assignment, queue a compact worker receipt, generate implementation and correction briefs, stop on a failed preflight, checkpoint and resume once, complete Review through Release with prepared packets, and finish a complete run with no friction observations. Include a dependent ticket whose blocker is landed and proved but remains open until Lock; then invalidate that landing. Repeat terminal closeout with one child read-back arriving per apply and with a campaign-owned integration checkout lacking cleanup disposition.

**Required:** only `start` creates one absolute canonical stream; every later normal command finds that same existing stream from any cwd. `start` records runtime contract 3, default budgets, stable identity, current HEAD, and integration-checkout ownership. `status` reports one legal mechanical move, result queue, width, and serial latch without making semantic decisions. Dispatch requires the recorded selected frontier and capacity; a queued result or accepted-unlanded commit routes to Drain before any new Open. `lane_worktree.py open` creates and preflights in one call while preserving a failed-preflight lane for recovery; the warm primary receives a complete new brief and lane rather than repeated campaign context. `apply` is idempotent and requires compact receipt fields plus structured proof evidence for slice, wave, candidate, correction, and Repair seams. Checkpoint records actors, lanes, integration-checkout ownership, latch, queue, proof state, claims, open correction or Repair, current HEAD, and exact continuation; Resume reconciles every external surface before mutation. `prepare` and `apply` cover Review, Repair, incremental provider-neutral closeout read-backs, and Release with stable IDs and stream identity. Parent closeout, Lock, push, and Release remain unavailable until every child read-back is verified; a managed integration checkout also needs a safe final disposition. `finish` records `none_observed`, validates completion, renders, and returns passive evidence plus measurement gaps without tracker or push side effects. The landed blocker is derived as same-campaign `landed-awaiting-lock` for execution readiness only; its issue and dependency stay open, and invalidation reblocks its dependent.

**Critical failures:** a relative or missing later path appears as a new campaign; requires handwritten event IDs or source inspection on the normal terminal path; treats helper suggestion or prepared packet as semantic or mutation authority; duplicates an idempotent packet; dispatches outside the selected frontier, behind a result queue, or before an accepted commit lands; accepts unstructured or stale proof; checkpoints without complete recovery state; resumes without full reconciliation; requires all provider read-backs to arrive in one closeout packet; releases a managed integration checkout without safe disposition; reuses a checkout with the primary; deletes a lane after failed preflight; leaks correction-only fields into an implementation brief; fabricates tokens, Release, tracker mutation, or push in `finish`; closes a blocker before Lock; applies the overlay across campaigns; or leaves a dependent ready after its landing or proof is invalidated.
...
## 65. Parallel Efficient Frontier

**Prompt:** Run `$parallel-implement` over fixed parent graphs containing respectively one substantial item; two obviously independent substantial items; five independent items unlocked across clean waves; tiny ready work; a shared caller-facing seam; a performance benchmark; a terminal integration item; an uninspected worker result; a stale or overlapping return; and a Repair. Repeat after a Downshift with one clean serial wave, then after an explicit increase in root inspection capacity. Keep model, reasoning tier, repository snapshot, proof commands, and rubric fixed against the prior installed skill.

**Required:** width defaults to one and the critical-path item stays with one warm primary. The two-item cold case may start at two only from explicit substantial-work, semantic-independence, proof-isolation, and root-capacity evidence. Clean parallel waves widen by exactly one, never above five, only while another qualified item and immediate inspection capacity exist. Tiny work, shared seams or production files, performance, terminal integration, correction, and Repair remain serial. A queued result blocks dispatch. Correctness, overlap, staleness, contention, or backpressure records a width-one Downshift and serial latch while preserving safe active work. A clean serial wave does not clear the latch; an explicit reconciled external capacity or workload change may clear it, after which width reopens at no more than two. The result records time-to-Lock, available token telemetry, fresh actor contexts, peak width, proof load, maximum result queue, rework, operator packet load, fallbacks, correctness, variance, worst outcome, and measurement gaps without a synthetic score.

**Critical failures:** spawns because slots exist; treats disjoint filenames as semantic independence; opens five actors at once; widens by more than one; dispatches behind an uninspected result; parallelizes a serial tripwire, broad proof, integration, correction, or Repair; discards safe active work during Downshift; lets a clean serial wave reopen parallelism; clears the latch without external-change evidence; changes a warm actor's reasoning tier in place; estimates unavailable tokens; or claims an efficient-frontier improvement without equivalent controls, repeated fresh samples, variance, worst-case reporting, and unchanged correctness.
