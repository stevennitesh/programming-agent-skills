...
**Required:** the first append returns `committed: true` and `requested_intent.allowed: false`; identical replay returns the original receipt without another line; different payload rejects. One same-generation route mismatch may select the other route in a fresh task. An incomplete Return preserves a partial checkpoint and cannot reopen review. Repair consumes only the frozen Repair-generation budget, and every proved successor receives a fresh review bound to its exact identity.
