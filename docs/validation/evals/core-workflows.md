...
**Required:** the baseline request routes to explicit `$audit-codebase` and completes one of three user objectives: Map, Audit, or Analyze. Map observes current repository provenance, maps every system and subsystem, assigns every shared unit one audit-owning subsystem plus named consumers, atomically publishes the sole durable linked HTML atlas under `.scratch/audit-codebase/`, and stops for user selection. It retains complete file ownership but renders one compact repository digest rather than a per-file hash ledger. A supplied report passes the contained-path, version, repository/run, unique-selection, and collision Entry Gate; an invalid explicit selection is never replaced with Map. Audit refreshes one selected subsystem's current Source Trace, always loads Reliability, Quality, and Defect owners, records all six mandatory lens dispositions, loads detailed Domain, Design, Simplification, Coding Practice, Performance, and declared owners when implicated, verifies every obtainable in-scope branch, and distinguishes defects, opportunities, retained complexity, and gaps. Analyze loads the candidate core plus implicated owners, rereads current source before confirming, changing, disproving, or blocking the candidate, treats a ready unsent questionnaire as an intermediate artifact rather than evidence, and folds `$codebase-design` Direct Design into a settled design or mixed candidate. Decision, returned-evidence, and successor routing load only when their branch applies. Audit and Analyze attempt one atomic replacement of all selected non-overlapping marked HTML regions and affected summaries through `update_report.py`; a publication failure preserves the report, triggers no retry, hand edit, or alternate mechanism, and returns completed source analysis immediately with `Report update: failed`. Neither objective ranks subsystems or starts downstream work. The ordinary PR routes to `$change-review`, the supported high-risk PR to `$high-assurance-review`, the bounded behavior-preserving edit to `$simplify-code`, and the uncertain symptom to `$diagnosing-bugs`.
