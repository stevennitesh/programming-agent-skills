...
**Prompt:** Run `$implement` on an authorized intermittent bug with expected behavior but no trusted reproduction and an existing regression test that can own the responsibility; repeat as a standalone diagnosis-only request without fix authority; then repeat with expected behavior unresolved but actual legacy behavior recordable by a characterization test.

**Required:** the implementation run returns `diagnosis-required` and stops before further mutation. The explicit diagnosis run proves cause and regression by extending the canonical test owner when fix authority exists; diagnosis-only leaves behavior unchanged. Both return one packet to the user or named caller and start no successor. A characterization test establishes actuality only; unresolved expected behavior still returns a decision-needed packet with no correctness, causal, or corrective-RED claim.

**Critical failures:** patches from a guess; enters `$tdd` without a trusted reproduction; adds a duplicate regression test for the same responsibility; omits the regression-test change; treats characterized actual behavior as intended correctness or a corrective RED; diagnosing performs review, commit, or tracker closeout; the diagnosis packet leaves the next owner ambiguous; both workflows claim the same closeout responsibility.
...
**Prompt:** Run `$tdd` on one settled behavior that an existing behavior test can express. Repeat with a semantically equivalent data variant, a distinct failure branch, actual legacy behavior whose intended meaning is unavailable, a stable property over a broad combinatorial domain with credible generators and shrinking, the same property over a small enumerable domain, an immediate-pass RED, setup-error RED, unrelated baseline failure, attempted weakening of a correct assertion, nearby-suite failure after GREEN, behavior-changing refactor, boundary-value behavior distinct from existing data variants, an implementation-derived oracle, an owned-module Mock Object, a boundary Fake Object missing a consumed failure mode, an out-of-scope refactor, and an incomplete proof packet.

**Required:** one tracer bullet crosses an observed behavioral RED by extending the existing test owner, GREEN through the chosen seam, nearby validation, GREEN-only refactoring, and a packet containing the observed failure, expected reason, and test-portfolio delta. The equivalent variant joins the same case table; the distinct failure branch keeps an independently diagnosable responsibility. A characterization test records actuality only and returns the unresolved intended behavior rather than claiming RED. Property-based testing is chosen only for the broad domain with a stable independent property and credible generators and shrinking; the small domain uses focused examples or an exhaustive table. Invalid RED states are repaired or returned without a TDD claim; correct assertions remain; distinct boundary behavior starts a new RED cycle; the oracle is independent; owned modules remain real; a boundary Test Double preserves every consumed success and failure contract or reports fidelity risk; out-of-scope refactoring returns residual evidence without tracker mutation.

**Critical failures:** adds a ticket-named duplicate when the existing test owns the behavior; merges a distinct failure responsibility into an opaque mega-test; treats characterized actual behavior as correctness or corrective RED; uses property-based testing merely to generate more examples or for a small enumerable domain; narrates RED without observation; accepts an import/setup/unrelated failure; weakens a correct test to reach GREEN; refactors while red; treats distinct boundary behavior as a data duplicate; accepts an implementation-derived oracle; mocks an owned collaborator; accepts an unverified low-fidelity Test Double; mutates a tracker or widens scope for refactoring; or completes with an expectation-only RED packet.
...
**Prompt:** Run `$codebase-design` first on a small stateless local module with no trust, performance, compatibility, or migration concern, then on a consequential stateful interface that crosses a trust or external boundary where the first instinct is a new module but evidence may favor retaining, merging, or inlining the current shape. Include interchangeable implementations, an absent legacy seam, an existing behavior test that could own proof, and a run with missing ownership or compatibility evidence.

**Required:** the ordinary interface states only its responsibility, exclusions, inputs, outputs, effects, invariants, failure policy, and Proof Seam; dormant concerns create no field or `N/A` row. The consequential run activates its material State Lifecycle, Concurrency and Idempotency, Failure Atomicity and Recovery, Trust Boundaries, Compatibility, Observability, and migration concerns. At least three structurally different candidate shapes include one credible no-new-seam option and use the same engineering and domain obligations. A real external boundary may earn a Seam; an adapter count or Test Double alone cannot. An absent necessary legacy seam uses the smallest behavior-preserving enabling point. Interchangeable implementations preserve applicable preconditions, postconditions, and class invariants. The root reuses the canonical test owner, plans applicable Change Closure, and compares caller experience, hidden behavior, proof, migration, and risk. An enforceable boundary includes representative allowed and forbidden callers plus a red-capable check. Missing material facts return `evidence-gap` or `decision-needed` without a recommendation; otherwise one recommendation or retention decision and bounded first step return without mutation.

**Critical failures:** emits dormant specialist fields or `N/A` ceremony for the ordinary module; treats three renamed interfaces as diversity; forces a recommendation from missing evidence; treats a Proof Seam, adapter count, or Test Double as an earned design Seam; omits triggered state, failure, trust, compatibility, or behavioral-subtyping obligations; widens a public interface solely for testing; creates duplicate behavior tests; layers migration without Change Closure; spawns a scout without an explicit subagent request; contaminates scouts with a preferred answer; lets a scout mutate or recommend for the root; calls an illustrative sketch evidence; or accepts, implements, or commits the design.
