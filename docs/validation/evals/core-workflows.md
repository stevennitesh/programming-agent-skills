...
**Prompt:** Run `$grill-with-docs` standalone on a named design with one confirmed domain term and a declined ADR. Repeat when the next decision needs an unavailable source fact. Separately verify that a bounded `$wayfinder` Chart ticket recommends the explicit composer and stops rather than invoking it. Then close a Wayfinder map whose settled decision changes durable domain language, first without and then with a current Domain Delta already accounting for that decision.
