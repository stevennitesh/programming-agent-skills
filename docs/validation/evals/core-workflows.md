...
an ordinary post-operation repair to `$implement`, and a hard post-operation
failure needing dedicated investigation to explicit `$diagnosing-bugs`.
...
**Prompt:** Run `$implement` on one ordinary deterministic bug whose cause is not yet known, then on an intermittent bug with no trusted reproduction. Run `$diagnosing-bugs` on the intermittent case once without fix authority and once with bounded fix authority. Repeat with expected behavior unresolved.

**Required:** Implement investigates and resolves the ordinary bug directly, but returns `diagnosis-required` when the intermittent case needs dedicated investigation. Explicit Diagnosis uses a practical exact-symptom loop and discriminating evidence. Diagnosis-only retains no source or runtime behavior change; any temporary local instrumentation is restored before Return. Fix mode applies only the authorized causal change and reruns the original loop. It adds a regression test only when policy requires it or the test is the cheapest durable protection at a correct seam. Every run returns to the user or named caller and starts no successor. Unresolved expected behavior returns to its decision owner with no causal claim or behavior change.

**Critical failures:** routes the ordinary deterministic bug away from Implement merely because its cause starts unknown; patches from a guess; enters `$tdd` without its explicit trigger; adds a shallow or ritual regression test; diagnosing performs review, commit, tracker closeout, or an unauthorized live effect; temporary instrumentation survives; or the result leaves the cause, evidence, fix, or remaining uncertainty ambiguous.
...
**Prompt:** Give the routing surfaces an ordinary deterministic local bug whose cause is initially unknown, then hard intermittent, performance, environment-only, production-only, and persistently ambiguous failures. Repeat with an explicit test-first request whose behavior and oracle are settled. In Diagnosis, include unrelated dirty hunks, an attempted fix that fails the original loop, no useful regression seam, and a strongest viable alternative with no discriminating probe result.

**Required:** the ordinary deterministic bug stays with Implement, and an explicitly test-first ordinary deterministic bug stays with TDD when behavior, symptom, and oracle are settled; an initially unknown cause alone does not bounce either case. The hard cases return `diagnosis-required`, and Skill Router recommends explicit `$diagnosing-bugs` without starting it. Diagnosis supports the cause with a predicted observation and a probe that distinguishes it from the strongest viable alternative. It minimizes only when useful, makes durable regression coverage conditional on policy or value, reruns the original symptom after a fix, and removes only its own failed attempt and instrumentation while preserving dirty hunks. An unresolved case returns the remaining viable explanation and missing evidence or next probe as incomplete.

**Critical failures:** routes the ordinary bug to Diagnosis merely because its cause starts unknown; routes a hard ambiguous failure to ordinary mutation; enters `$tdd` without its explicit trigger; alternates between skills on the same facts; claims cause without discriminating evidence; mandates a ledger, exhaustive minimization, or regression test without its condition; or patches before the cause is supported.
...
**Required:** a recognized conflict completes `State -> Trace -> Return` as `inspection`, reports both authorities and exact remaining state, and changes nothing. Plausible markers are inspected rather than assumed; intentional literals and no-conflict states return `route mismatch`. The resolved diff belongs to review, an ordinary post-operation repair to Implement, and a hard post-operation failure needing dedicated investigation to Diagnosis. Read-only completion does not require Reconcile, Prove, or Finish.
...
**Prompt:** Run `$simplify-code` on a named target that duplicates an existing owner, wraps a language or platform capability, keeps an unsupported compatibility path, and sits beside required trust-boundary behavior. Provide one useful caller-facing check. Repeat from an `$audit-codebase` pickup; without a target; with broad discovery, new behavior, a hard bug, or an unsettled Interface decision; with a proposed one-line change that moves complexity into callers; with no worthwhile safe cut; with a failed proof in a pre-dirty worktree; and with an explicitly requested repeated pass in the same target.

**Required:** only the user-selected target or a target selected in an Audit pickup proceeds. Simplify Code reads the target and real callers, names accepted behavior and the complexity to remove, then chooses a coherent reduction with a preference for deletion, reuse, native capability, collapsing an unearned layer, and simpler control flow or state. Fewer lines do not win when complexity moves elsewhere. It migrates affected callers, removes displaced paths, preserves required boundary behavior and unrelated work, runs the nearest useful check through the real caller or artifact, and inspects the final diff. Before-and-after comparison and broader proof appear only when the claim, repository policy, shared impact, or concrete risk requires them. Missing or mismatched targets, bug repair, foreign outcomes, and unsettled decisions return unchanged. No worthwhile safe cut returns a truthful no-change result only after the current owner and callers expose no credible proved reduction. A failed check returns the exact current state and recovery without claiming completion. An explicitly requested repeated pass stays in the same target and proves each reduction before choosing the next. Every result starts no successor.

**Critical failures:** invents or widens the target; changes accepted behavior; treats fewer lines as proof; moves equivalent complexity into callers, configuration, tests, or another wrapper; keeps an unsupported old path; adds speculative protection; weakens required trust-boundary behavior; demands a new test, before-and-after harness, full suite, ledger, budget, or status packet without its trigger; changes unrelated work; stages, commits, mutates external state, or starts a successor; claims a reduction without evidence capable of detecting the preserved behavior; or claims no safe cut merely because the first idea failed.
