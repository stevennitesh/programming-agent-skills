...
acceptance and supported state branch. Hold one independently understandable,
acceptance-complete, testable change: include every coupled surface necessary
to its coherent proof story and Return adjacent concerns without reopening
caller-owned scope.
...
connector, apply Mutation read-back, and release the claim. On ambiguous,
partial, or failed closeout, preserve the verified commit as the recovery
anchor; refetch actual connector state; classify each intended effect as
`applied`, `missing`, `conflicted`, or `unknown`; and resume only configured
missing steps whose owner can establish safe replay. Otherwise Return the
commit identity, observations, classifications, and one owner-matched safe
continuation without blind replay or commit rollback. Local Markdown and
...
======================
{
  "fixture": {
    ...
    "id": "Q2-PC-01",
    "initial_state": [
      "Item was open with label active and claimed."
    ],
    "mutation_boundary": [
      "configured closeout fields"
    ],
    "observations": [
      "Refetch shows label done applied, K missing, claim held, no concurrent field change."
    ],
    "permitted_tools": [
      "simulated refetch",
      "simulated safe missing-step mutation"
    ],
    "requested_output": "Classify every intended effect and select only safe missing steps.",
    "source_facts": [
      "Commit C is verified.",
      "Intended effects are label done, comment key K, and claim release.",
      "Queries for labels, K, and claim are supported.",
      "Missing comment and claim release are independently safe when current state is unchanged."
    ],
