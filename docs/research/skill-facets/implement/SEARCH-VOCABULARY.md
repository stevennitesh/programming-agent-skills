...
The reusable source-distillation workflow lives in
`../../../synthesis/methods/source-distillation-flow.md`. The implement-specific
facet map lives in `README.md`. This file is only for keywords, exact queries,
source anchors, and noise filters that may be reused across multiple implement
facets; do not treat its section order as the current facet order.
